    ...
    <div v-for="(set, j) in modelValue" :key="j" class="flex items-center gap-2.5" :class="readonly ? 'bg-gym-gray-50 rounded-xl p-3' : ''">
      ...
      <template v-if="readonly">
        <div class="flex-1 flex items-center gap-2 justify-center">
          ...
          <span v-if="set.seconds" class="text-xs text-gym-gray-300 mx-0.5">/</span>
          <span v-if="set.seconds" class="text-sm font-bold text-gym-gray-900 tabular-nums">{{ set.seconds }}<span class="text-xs font-normal text-gym-gray-400">s</span></span>
        </div>
      </template>
      <template v-else>
        ...
        <div class="flex-1 flex flex-col gap-1 min-w-0">
          <label class="text-[11px] font-medium text-gym-gray-500 px-1">Seg</label>
          <input :value="set.seconds" @input="updateSeconds(j, $event)" type="number" inputmode="numeric" class="input-field-sm text-center tabular-nums" placeholder="seg" style="font-size: 16px" :aria-label="`Segundos serie ${j + 1}`" />
        </div>
